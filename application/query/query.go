...
func QueryListProduct(skus []float64, printResponse bool) (*esquery.SearchRequest, error) {
...
func QueryAggSku(key string, printResponse bool, status_ids []int, product_status_ids []int) (*esquery.SearchRequest, error) {
	...
	if len(status_ids) > 0 {
		for _, v := range status_ids {
			match = append(match, esquery.Match("status_id", v))
		}
	}
	if len(product_status_ids) > 0 {
		for _, v := range product_status_ids {
			match = append(match, esquery.Match("product_status_id", v))
		}
	}
	if len(match) == 0 {
